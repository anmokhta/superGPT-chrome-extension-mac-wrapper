/* global showConfirmDialog, noConversationFolderElemet, toast, getLastSelectedConversationFolder, deleteConversation, rgba2hex, deleteAllConversations, openExportModal, removeConversationElements, translate, errorUpgradeConfirmation, debounce, createTooltip, closeMenus, fetchConversations, noConversationElement, resetConversationCounts, conversationFolderElement, addNewConvFolderElementToSidebar, selectedConversationFolderBreadcrumb, openMoveConvFolderModal, fetchSidebarConversations, adjustMenuPosition, openFolderProfileSelectorModal, cachedSettings */
// eslint-disable-next-line no-unused-vars
async function showConversationManagerFolderMenu(folderSettingsElement, folder, sidebarFolder = false, leftMenu = false) {
  ...
  addConversationManagerFolderMenuEventListeners(folder, sidebarFolder);
...
async function addConversationManagerFolderMenuEventListeners(folder, sidebarFolder = false) {
  const folderId = folder.id;
  ...
  const moveFolderConversationFolderButton = document.querySelector(`#move-folder-conversation-folder-button-${folderId}`);
  const colorConversationFolderButton = document.querySelector(`#color-conversation-folder-button-${folderId}`);
  const setProfileFolderConversationsButton = document.querySelector(`#set-profile-folder-conversations-button-${folderId}`);
  ...
  if (viewModeConversationFolderButton) {
    createTooltip(viewModeConversationFolderButton, cachedSettings.excludeConvInFolders ? 'Show all conversations' : 'Only show conversations not in folders', 'transform: translate(95%, -20%);');
  }
  ...
  moveFolderConversationFolderButton?.addEventListener('click', () => {
  ...
  colorPickerButton?.querySelector('input[id^=color-picker-input-]')?.addEventListener('input', (e) => {
    ...
    updateConversationFolderIndicators(folderId, newData);
  ...
  setProfileFolderConversationsButton?.addEventListener('click', (e) => {
    e.stopPropagation();
    e.preventDefault();
    closeMenus();
    ...
    fileInput.onchange = async () => {
      ...
      reader.onload = async (e) => {
        ...
        chrome.runtime.sendMessage({
          ...
          // update folder image from data:url to url, so when creating subfolder, we use the right image
          curFolderImages.forEach((el) => {
            el.src = data.image;
          });
        });
  ...
  deleteConversationFolderButton?.addEventListener('click', () => {
    showConfirmDialog('Delete Folder', 'Are you sure you want to delete this folder? All conversations and sub folders inside this folder will be deleted too.', 'Cancel', 'Delete Folder', null, () => confirmDeleteConversationManagerFolder(folder, sidebarFolder), 'red', false);
  });
  ...
  clearAllConversationFolderButton?.addEventListener('click', () => {
    ...
    showConfirmDialog(dialogTitle[folderId], dialogSubtitle[folderId], 'Cancel', 'Confirm', null, () => {
      // click on root in the sidebar
      document.querySelector('#sidebar-folder-drawer #folder-breadcrumb-root')?.click();

      if (folderId === 'all' || selectedConversationFolderBreadcrumb.map((f) => f.id).includes(folderId)) {
...
function confirmDeleteConversationManagerFolder(folder, sidebarFolder = false) {
  const folderId = folder.id;
  ...
  if (sidebarFolder) {
    ...
    if (defaultConversationFolders && !defaultConversationFolders.nextElementSibling) {
      defaultConversationFolders.insertAdjacentElement('afterend', noConversationFolderElemet());
    }
  } else {
    const firstFolder = document.querySelector('#conversation-manager-sidebar-folders > div[id^="conversation-folder-wrapper-"]');
    if (firstFolder) {
      // if selectedFolder is deleted, select the first folder
      if (selectedConversationFolderBreadcrumb.map((f) => f.id).includes(folderId)) {
        firstFolder.click();
      }
    } else {
  ...
  const parentFolderId = folder.parent_folder;
...
function updateConversationFolderNameElement(conversationFolderNameElement, folderId, newName) {
  ...
  const folderBreadcrumbs = document.querySelectorAll(`#folder-breadcrumb-${folderId}`);
  folderBreadcrumbs.forEach((el) => {
    el.innerText = newName;
  });
  selectedConversationFolderBreadcrumb.forEach((folder) => {
...
function updateConversationFolderIndicators(folderId, newData) {
  ...
  conversationCardsInFolder.forEach((conversationCard) => {
    ...
    if (conversationCardFolderColorIndicator && Object.keys(newData).includes('color')) {
      ...
      conversationCardFolderColorIndicator.title = newData.name || '';
    }
  });
}
